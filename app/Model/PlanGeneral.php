...
class PlanGeneral extends Plan implements PlanInterface
{
    ...
    public function saveAll($request)
    {
        ...
        if (isset($request->email_verification_servers)) {
            ...
            // add new verification servers
            foreach ($request->email_verification_servers as $serverUid => $param) {
                if ($param['check'] != 'no') {
                    $server = \Acelle\Model\EmailVerificationServer::findByUid($serverUid);
                    $planServer = new \Acelle\Model\PlansEmailVerificationServer();
                    $planServer->plan_id = $this->id;
                    $planServer->server_id = $server->id;
                    $planServer->save();
                }
            }
        }
