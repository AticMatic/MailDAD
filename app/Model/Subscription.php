...
class Subscription extends Model
{
    ...
    public function planVerification()
    {
        // @todo dependency injection
        return $this->belongsTo(PlanVerification::class, 'plan_id')
            ->where('type', PlanVerification::TYPE_VERIFICATION);
    }
    ...
    public function createNewSubscriptionInvoiceWithoutTrial()
    {
        ...
        $invoice->no_payment_required_when_free = $this->plan->no_payment_required_when_free;
    ...
    public function activate()
    {
        \DB::transaction(function () {
            ...
            // already have active subscription exception
            switch ($this->plan->type) {
                case \Acelle\Model\PlanGeneral::TYPE_GENERAL:
                    if ($this->customer->getCurrentActiveGeneralSubscription()) {
                        throw new \Exception("Customer already has an active general subscription!");
                    }

                    break;
                case \Acelle\Model\PlanVerification::TYPE_VERIFICATION:
                    if ($this->customer->getCurrentActiveVerificationSubscription()) {
                        throw new \Exception("Customer already has an active verification subscription!");
                    }
    ...
    {
        if ($plan->type == PlanGeneral::TYPE_GENERAL && $customer->getNewOrActiveGeneralSubscription()) {
    ...
    public static function scopeByPlanType($query, $type)
    {
        $query = $query->whereHas('plan', function (Builder $q) use ($type) {
            $q->where('type', $type);
        });
    }
    ...
    {
        $query = $query->whereHas('planVerification', function (Builder $query) {
            $query->verification();
        });
    }
