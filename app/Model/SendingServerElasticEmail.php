...
class SendingServerElasticEmail extends SendingServer implements DomainVerificationInterface
{
    ...
    public const DNS_SPF = 'v=spf1 a mx include:_spf.elasticemail.com ~all';
    ...
    // ELasticEmail Official SDK has a very strange PHP SDK
    // It works like this
    //
    //    $config = new Elastic\Config('API KEY');
    //    $instance = new CampaignInstance($config)   // or DomainInstance, or o
    public function client2($instance)
    {
        if (!$this->client2) {
            $this->client2 = new \ElasticEmail\ElasticEmail($this->api_key);
        }
    ...
    private function curlExecute($method, $action, $params = [])
    {
        $url = "https://api.elasticemail.com/v4/{$action}/";
        ...
        // Handle errors
        if (curl_errno($ch)) {
            throw new \Exception("Cannot execute '{$action}' using Elastic API, curl error: ".curl_error($ch));
        }

        if ($result === false || empty($result)) {
            throw new \Exception("Cannot execute '{$action}' using Elastic API, curl_exec returns false or empty");
        }
    ...
    {
        // @important: do not use the GET /domains/$name (individual domain name) API
        // as it returns ERROR in case the speicified domain does not exist
        // it messes up with the case when there is another error with the request (invalid key, timeout...)

        // So, retrive the list of domains and check one by one

        $existed = false;
        foreach ($this->getAllDomains() as $r) {
            if ($r->Domain == $domain) {
                return $r;
            }
        }
    ...
    {
        $action = "domains";
        $allDomains = $this->curlExecute('GET', $action);
        return $allDomains;
    }

    public function addDomain(string $domain): array
    {
        if ($this->getDomainInfo($domain)) {
            ...
            $result = $this->curlExecute('POST', $action = 'domain', $params);
        }

        // ElasticEmail API response does not include DNS info, resorted to get those from dashboard and hard-code them here
        return [
            'identity' => [
                'name' => 'tracking',
                'type' => 'CNAME',
            ...
            'dkim' => [
                [ 'name' => 'api._domainkey' ], // Convention here: 'value' missing/empty means not available in the web UI
            ],
    ...
    // Interface 2/2: check verification status
    public function checkDomainVerificationStatus(SendingDomain $domain): array
    {
        $action = "domains/{$domain->name}/verification";
        $info = $this->curlExecute('PUT', $action, "None");

        $identity = !$info->Verify; // TRUE does not mean OK, instead, it means there are more to verify (i.e. not fully verified)
        $dkim = $info->Dkim; // DKIM is considered done if DMARC is also done
        $spf = $info->Spf;
        $dmarc = $info->DMARC;
        $finalStatus = $identity && $dkim && $spf;

        // Notice: in the API response, there is no separate verification result for 'tracking CNAME api.elasticemail.com'
        // As a result, even if the DNS is set, there is no way to know, so, we simply show 'pending' to users
        // Don't worry, it will turn to 'verified' if all DKIM, SPF and DMARC are all verified

        return [ $identity, $dkim, $spf, $dmarc, $finalStatus ];
    }
