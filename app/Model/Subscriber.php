...
class Subscriber extends Model
{
    ...
    /**
     * Blacklist a subscriber GLOBALLY
     *
     * @return bool
     */
     ...
     * @return bool
     */
    public function sendToUserBlacklist($reason, $customerId)
    {
        // blacklist all email
        self::where('email', $this->email)->update(['status' => self::STATUS_BLACKLISTED]);
    ...
    public function doCopy(MailList $list, Closure $duplicateCallback = null)
    {
        ...
        // Search for fields in the target $list that have the same tag value as original $this' list
        $sourceTags = $this->mailList->fields()->pluck('tag');
        $matchedFields = $list->fields()->whereIn(
            'tag',
            $sourceTags,
        )->get();

        foreach($matchedFields as $field) {
            $value = $this->getValueByTag($field->tag);
    ...
    // Load a form, click Submit to actually opt out
    public function generateUnsubscribeFormWithoutMessageId($absoluteUrl = true)
    {
        $fakeMessageId = StringHelper::generateMessageId('localhost.local', $this->mailList->customer->uid);
        return $this->generateUnsubscribeForm($fakeMessageId, $absoluteUrl);
    }
    ...
    {
        if (!static::columnExists($fieldName)) {
            Schema::table(static::getTableName(), function (Blueprint $table) use ($fieldName) {
    ...
    public static function columnExists($fieldName)
    {
        // table name without prefix
        $tableName = static::getTableName();

        return has_column($tableName, $fieldName);
    }
    ...
    {
        return !is_null(static::find($this->id));
    }

    public function translateIncomingWebhookTags()
    {
        $tags = [];
