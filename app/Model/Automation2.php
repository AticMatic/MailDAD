...
use Acelle\Library\Automation\WaitUntil;
...
class Automation2 extends Model
{
    ...
    public function subscribersNotTriggeredThisYear()
    {
        ...
        return $this->subscribersToSend()->whereNotIn('subscribers.id', function ($query) use ($thisYear) {
            $query->select('subscriber_id')
                  ->from('auto_triggers')
                  ->whereNotNull('subscriber_id')  // especially important when it is in a NOT IN sub query!!!!!!!!!
    ...
    {
        $timeToTrigger = $this->getTriggerAction()->getOptions()['at'];
        ...
        $triggerTime = new DateTime($timeToTrigger, new DateTimeZone($this->customer->timezone));

        if ($currentTime < $triggerTime) {
            $timeDiff = "Quit, current time {$currentTime->format('H:i:s')} < {$triggerTime->format('H:i:s T (P)')}";
        } else {
            $timeDiff = "Ready to kick off, current time {$currentTime->format('H:i:s')} > {$triggerTime->format('H:i:s T (P)')}";
        }

        // Get the modify interval: 1 days, 2 days... for example
        $interval = $this->getTriggerAction()->getOptions()['before'];

        $dobToTrigger = Carbon::now($this->customer->timezone)->modify($interval);
        $dobToTriggerStr = $dobToTrigger->format($formatCarbon);
        ...
        $query = $this->subscribers()
            ...
            ->addSelect(
                DB::raw("'{$timeDiff}' as timediff")
            )
            ->addSelect(
                DB::raw("'{$dobToTriggerStr}' AS dob_to_trigger")
            )
            ->addSelect(
                DB::raw("'$interval' AS date_modify")
            )
            ->addSelect(
                DB::raw(
                    // Make it the same year 2024 to count date diff
                    "DATEDIFF( DATE_FORMAT(STR_TO_DATE(subscribers.{$dobField}, '".config('custom.date_format_sql')."'), '2024-%m-%d') , DATE_FORMAT('{$dobToTriggerStr}', '2024-%m-%d')) AS datediff"
                )
            )
            ->leftJoin('auto_triggers', function ($join) {
                $join->on('auto_triggers.subscriber_id', 'subscribers.id');
                $join->where('auto_triggers.automation2_id', $this->id);
            });

        return $query;
    }

    // @important: This method must be executed at least EVERY DAY
    //             Because it only triggers for subscribers whose Date of Birth is on the same date (with modify x day before)
    ...
    public function getActionStats($attributes)
    {
        ...
        } elseif ($action->getType() == 'ElementWaitUntil') {
            // Count the numbe contacts with this action as last executed one
            $queue = $this->subscribersByLatestAction($action->getId())->count();
    ...
    public function getAction($attributes): Action
    {
        switch ($attributes['type']) {
            ...
            case 'ElementWaitUntil':
                $instance = new WaitUntil($attributes);
                break;
    ...
    public function getSubscribersWithTriggerInfo()
    {
        ...
        // Trick: triggers whose subscribers are deleted. Keep them here for reporting to the web UI
        ...
        // Hack: to better debug in case of DATE-OF-BIRTH trigger

        if ($this->isDateOfBirthTrigger()) {
            $timeToTrigger = $this->getTriggerAction()->getOptions()['at'];
            $formatSql = config('custom.date_format_sql');
            $formatCarbon  = 'Y-m-d'; // DO NOT READ FROM config(), as it is used here only
            ...
            if ($currentTime < $triggerTime) {
                $timeDiff = "now {$currentTime->format('H:i:s')} < {$triggerTime->format('H:i P')}";
            } else {
                $timeDiff = "now {$currentTime->format('H:i')} > {$triggerTime->format('H:i:s P')}";
            }
            ...
            ->addSelect(
                DB::raw("'{$dobToTrigger->format('M-d')}' AS dob_to_trigger")
            )
            ->addSelect(
                DB::raw("'$interval' AS date_modify")
            )
            ...
            $noSubscribers->addSelect(DB::raw("'' as dob"));
            $noSubscribers->addSelect(DB::raw("'' as timediff"));
            $noSubscribers->addSelect(DB::raw("'' as dob_to_trigger"));
            $noSubscribers->addSelect(DB::raw("'' as date_modify"));
            $noSubscribers->addSelect(DB::raw("'' as datediff"));
        }

        return $withSubscribers->union($noSubscribers);
    }

    // A special check for DateOfBirth case
    public function isDateOfBirthTrigger()
    {
        $triggerName = $this->getTriggerAction()->getOption('key');
        return $triggerName == static::TRIGGER_TYPE_SAY_HAPPY_BIRTHDAY;
    }
