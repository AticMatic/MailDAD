...
class PageController extends Controller
{
    ...
    public function signUpForm(Request $request)
    {
        ...
        $logger = applog("optin-for-customer-{$customerUid}-list-{$list->uid}");

        // Language
        if ($list->customer->language) {
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                $logger->info('Opting in through default opt-in page '.$request->EMAIL);
                list($validator, $subscriber) = $list->subscribe($request, MailList::SOURCE_WEB, $logger);
            } catch (\Exception $ex) {
                $logger->info('Failed to opt in: '.$request->EMAIL);
            ...
            $logger->info('Done opting in: '.$subscriber->email);

            // Timeline record
            \Acelle\Model\Timeline::recordSignUpFormOptIn($subscriber);

            if ($request->redirect_url) {
                $logger->info("Redirecting {$subscriber->email} to {$redirect_url}");
                ...
                $logger->info("Redirect {$subscriber->email} to ThankYou page, asking user to check email to confirm (double opt-in)");
                ...
                // All done, confirmed
                $logger->info("All done, redirect {$subscriber->email} to PageController@signUpConfirmationThankyou");
