...
class SubscriptionController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($subscription->isNew()) {
            ...
            // reflash
            $request->session()->reflash(); // Keeps flash data for the next request too
    ...
    public function payment(Request $request)
    {
        ...
        if ($invoice->getLastAndItIsPendingTransaction()) {
            return view('subscription.pending', [
                'invoice' => $invoice,
                'transaction' => $invoice->getLastAndItIsPendingTransaction(),
            ]);
        }
    ...
    public function checkout(Request $request)
    {
        ...
        if (
            ...
            in_array($paymentGateway->type, ['stripe', 'braintree', 'paystack']) && // @todo moving this to interface
    ...
    public function cancelNow(Request $request)
    {
        ...
        if ($request->user()->customer->can('cancelNow', $subscription)) {
            ...
            Hook::execute('subscription_cancelled', [$subscription]);
        }
    ...
    public function verifyPendingTransaction(Request $request, $invoice_uid)
    {
        ...
        // get gateway
        // @todo: payment_version2
        $paymentGateway = $transaction->paymentMethod->paymentGateway;

        // gateway verify transaction from invoice
        try {
            // get transaction result from service
            $paymentGateway->getService()->verify($transaction);
        } catch (\Throwable $e) {
            $request->session()->flash('alert-error', $e->getMessage());
            return redirect()->action('SubscriptionController@payment', [
                'invoice_uid' => $invoice->uid,
