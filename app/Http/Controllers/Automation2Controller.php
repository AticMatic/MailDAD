...
class Automation2Controller extends Controller
{
    ...
    public function waitUntilCreate(Request $request, $uid)
    {
        // init automation
        $automation = Automation2::findByUid($uid);
        ...
        return view('automation2.waitUntilCreate', [
            'automation' => $automation,
            'element' => $automation->getElement(),
        ]);
    }
    ...
    public function waitUntilSave(Request $request, $uid)
    {
        // init automation
        $automation = Automation2::findByUid($uid);
        ...
        $datetime = \Carbon\Carbon::createFromFormat('Y-m-d H:i', $request->date . ' ' . $request->time, $customer->timezone);

        return response()->json([
            'status' => 'success',
            'message' => trans('messages.automation.action.added'),
            'title' => trans('messages.automation.wait_until', [
                'datetime' => $datetime->format('Y-m-d H:i:s T'),
            ]),
            'options' => [
                'wait_until' => $datetime->toIso8601String(),
            ],
        ]);
    }

    public function waitUntilEdit(Request $request, $uid)
    {
        // init automation
        $automation = Automation2::findByUid($uid);
        $customer = $request->user()->customer;
        ...
        if ($request->isMethod('post')) {
            ...
            return response()->json([
                ...
                'title' => trans('messages.automation.wait_until', [
                    'datetime' => $datetime->format('Y-m-d H:i T'),
                ]),
                'options' => [
                    'wait_until' => $datetime->toIso8601String(),
                ],
        ...
        return view('automation2.waitUntilEdit', [
            'key' => $request->key,
            'automation' => $automation,
    ...
    public function emailAttachmentDownload(Request $request, $uid, $email_uid, $attachment_uid)
    {
        ...
        return response()->download($attachment->getPath(), $attachment->name);
    }
    ...
    {
        $automation = Automation2::findByUid($uid);
        $httpConfig = $request->user()->customer->local()->newHttpConfig();
    ...
    public function outgoingWebhookSetup(Request $request, $uid, $http_config_uid)
    {
        //
        $automation = Automation2::findByUid($uid);
        $httpConfig = \Acelle\Model\HttpConfig::findByUid($http_config_uid);
    ...
    public function outgoingWebhookSave(Request $request, $uid, $http_config_uid)
    {
        $automation = Automation2::findByUid($uid);
        $httpConfig = \Acelle\Model\HttpConfig::findByUid($http_config_uid);
        ...
        $httpConfig->fillAttributes($request->webhook);
        $httpConfig->save();
    ...
    public function outgoingWebhookShow(Request $request, $uid)
    {
        ...
        // null
        $httpConfigUid = null;
        if ($element && $element->get('options') && isset($element->get('options')->http_config_uid)) {
            $httpConfigUid = $element->get('options')->http_config_uid;
        }
        ...
        if (!$httpConfigUid) {
            ...
            $httpConfig = \Acelle\Model\HttpConfig::findByUid($httpConfigUid);
        }
    ...
    public function outgoingWebhookTest(Request $request, $uid, $http_config_uid)
    {
        $automation = Automation2::findByUid($uid);
        $httpConfig = \Acelle\Model\HttpConfig::findByUid($http_config_uid);
        ...
        if ($request->isMethod('post')) {
            // just test
            $httpRequest = $httpConfig->test([
                'automation_id' => $automation->uid,
            ], $request->webhook);

            return view('http_configs.testResult', [
                'webhook' => $httpConfig,
                'httpRequestLog' => $httpRequest->httpRequestLogs()->latest()->first(),
            ]);
        }

        return view('automation2.outgoingWebhookTest', [
            'automation' => $automation,
            'httpConfig' => $httpConfig,
        ]);
    }
