...
use Acelle\Library\Exception\DistributedModeWaitForMaster;
use Cache;
...
use function Acelle\Helpers\has_column;
...
trait HasTemplate
{
    ...
    public function setTemplate($template, $name = null)
    {
        ...
        if (has_column($this->getTable(), 'plain')) {
    ...
    public function prepareEmail($subscriber, $server, $fromCache = false, $expiresInSeconds = 600)
    {
        ...
        if ($this->sign_dkim && $server && $server->allowClientToSignDkim()) {
        ...
        $this->prepareAttachments(function ($att, $attachment) use (&$message) {
            // Finally
            $message->attach($attachment);

            // This is used by certain delivery services like ElasticEmail
            $message->extAttachments[] = [ 'path' => $att->getPath(), 'type' => $attachment->getContentType()];
        });

        return array($message, $msgId);
    }

    public function prepareAttachments(\Closure $callback)
    {
        // Email/Campaign model
        foreach ($this->attachments as $att) {
            if (config('custom.distributed_mode')) {
                // If in distributed mode, then write/read from cache

                if (!$att->cacheExistsInRedis()) {
                    // Dispatch a job to a queue that runs on master server
                    throw new DistributedModeWaitForMaster('Wait for job CacheAttachments to CACHE attachments first');
                }

                // Retrieve attachments from CACHE
                // Notice that this is a performance issue as workers have to "download" cache from the master Redis
                // Consider making a local cache instance too!
                $attachment = $att->makeSwiftAttachmentFromRedis();
            } else {
                if (!file_exists($att->getPath())) {
                    throw new \Exception('Attachment file does not exist at '.$att->getPath());
                }

                $attachment = $att->makeSwiftAttachmentFromPath();
            }

            $callback($att, $attachment);
        }
    }
    ...
    public function getHtmlContent($subscriber = null, $msgId = null, $server = null, $fromCache = false, $expiresInSeconds = 600, $tagValues = [])
    {
        ...
        $pipeline->add(new TransformUrl($this->template, $msgId, $this->trackingDomain, $trackClick = $this->track_click));
    ...
    public function uploadAttachment($file)
    {
        ...
        $att->customer_id = $this->customer->id;
