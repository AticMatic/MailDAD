...
class Domain
{
    ...
    {
        $hostname = "{$this->getFullDkimSelector()}.{$this->name}"; // For example "{selector}._domainkey.example.com."
    ...
    {
        $dnsValue = $this->getDkimDnsRecord();

        if (Setting::isYes('sending-domain.use_dots_and_quotes')) {
            $dnsValue = doublequote($dnsValue);
        }
        return [
            'type' => 'TXT',
            'name' => $this->getFullDkimHostName(),
            'value' => doublequote($dnsValue),
        ];
    }
}
