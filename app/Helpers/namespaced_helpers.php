...
function has_column($tableWithoutPrefix, $fieldName)
{
    try {
        return \Schema::hasColumn($tableWithoutPrefix, $fieldName);
    } catch (\Exception $ex) {
        // MariaDB 11 may return an error with Schema::hasColumn
    }

    $rawTableName = \DB::getTablePrefix().$tableWithoutPrefix;
    return count(DB::select("SHOW COLUMNS FROM `{$rawTableName}` LIKE '{$fieldName}'")) > 0;
}

function geoip_resolve($ipAddress)
{
    // Might throw exception like:
    // - GeoIp2\Exception\AddressNotFoundException  The address 127.0.0.1 is not in the database.
    // - InvalidArgumentException  The value "1.2.3.43333" is not a valid IP address.

    $dbsource = base_path(\Acelle\Model\Setting::get('geoip.sqlite.dbname'));

    if (!file_exists($dbsource)) {
        throw new \Exception("GeoIP DB does not exist at ".$dbsource);
    }

    $geoip = new \GeoIp2\Database\Reader($dbsource);
    $city = $geoip->city($ipAddress);
...
{
    return mb_encode_numericentity(htmlspecialchars_decode(htmlentities($string, ENT_NOQUOTES, 'UTF-8', false), ENT_NOQUOTES), [0x80, 0x10FFFF, 0, ~0], 'UTF-8');
}

function transform_tags(array $tags, $text)
{
    $search = [];
    $replace = [];
