...
// List of constants
// Job types
define('ACM_QUEUE_TYPE_DEFAULT', 'default');  // must be executed on the same server as Nginx/Apache (cache attachment for example)
...
function paginate_query($query, $perPage, $orderBy, Closure $callback, $maxPageToLoad = null)
{
    ...
    for ($page = 1; $page <= $pages && (is_null($maxPageToLoad) || $page <= $maxPageToLoad); $page++) {
        $offset = ($page - 1) * $perPage;

        $pagedQuery = clone $query;

        if (!is_null($orderBy)) {
            // Notice that adding order By might significantly DECREEASE performance: from 1s -> 24s!!!!
            $pagedQuery = $pagedQuery->orderBy($orderBy);
        }

        $callback($page, $pagedQuery->limit($perPage)->offset($offset));
    }
}
...
function distributed_mode_check()
{
    if (config('custom.distributed_mode')) {
        if (config('cache.default') != 'redis') {
            throw new \Exception('Application is in DISTRIBUTED mode, cache driver must be set to "redis" (currently "'.config('cache.default').'")');
        }

        if (!redis_enabled()) {
            throw new \Exception('Application is in DISTRIBUTED mode but Redis is not enable');
        }
    }
