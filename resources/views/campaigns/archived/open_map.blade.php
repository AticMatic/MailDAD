...
function initialize() {        
	...
	@if ($campaign->campaignArchives()->whereNotNull('open_ip_address')->count() > 0)
		@foreach ($campaign->campaignArchives()->whereNotNull('open_ip_address')->get() as $key => $archive)
			@php
				$location = $archive->openIpLocation;
			@endphp
