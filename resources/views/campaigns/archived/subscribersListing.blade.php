            ...
            <tr class="position-relative">
                ...
                <td>
                    <div class="no-margin text-bold">
                        <a class="kq_search" href="{{ action('SubscriberController@edit', ['list_uid' => $subscriber->mailList->uid ,'id' => $subscriber->id]) }}">
                            {{ $subscriber->email }}
                ...
                <td class="text-end text-nowrap pe-0">
                    ...
                    <a href="{{ action('SubscriberController@edit', ['list_uid' => $subscriber->mailList->uid, "id" => $subscriber->id]) }}" role="button" class="btn btn-secondary btn-icon">
						...
						<ul class="dropdown-menu dropdown-menu-end">
							@if (\Gate::allows('subscribe', $subscriber))
								<li><a class="dropdown-item list-action-single"
                                    link-method="POST"
                                    href="{{ action('SubscriberController@subscribe', ['list_uid' => $subscriber->mailList->uid, "ids" => $subscriber->id]) }}"><span class="material-symbols-rounded">mark_email_read</span> {{ trans('messages.subscribe') }}</a></li>
							@endif
							@if (\Gate::allows('unsubscribe', $subscriber))
								<li><a class="dropdown-item list-action-single"  link-method="POST" href="{{ action('SubscriberController@unsubscribe', ['list_uid' => $subscriber->mailList->uid, "ids" => $subscriber->id]) }}"><span class="material-symbols-rounded">logout</span> {{ trans('messages.unsubscribe') }}</a></li>
							@endif
							...
							<li>
								<a href="#copy" class="dropdown-item copy_move_subscriber"
									data-url="{{ action('SubscriberController@copyMoveForm', [
										'ids' => $subscriber->id,
										'from_uid' => $subscriber->mailList->uid,
										'action' => 'copy',
									]) }}">
							...
							@if (\Gate::allows('update', $subscriber))
								<li>
                                    ...
                                    href="{{ action('SubscriberController@resendConfirmationEmail', ['list_uid' => $subscriber->mailList->uid, "ids" => $subscriber->id]) }}">
							...
							@if (\Gate::allows('delete', $subscriber))
								<li><a class="dropdown-item list-action-single" link-confirm="{{ trans('messages.delete_subscribers_confirm') }}" href="{{ action('SubscriberController@delete', ['list_uid' => $subscriber->mailList->uid, "ids" => $subscriber->id]) }}"><span class="material-symbols-rounded">delete_outline</span> {{ trans("messages.delete") }}</a></li>
							@endif	
						</ul>
					</div>
                </td>
