			...
			<tr>
				...
				<td>
					<span class="no-margin kq_search url-truncate">
						<a title="{{ $item->click_url }}" href="{{ $item->click_url }}" target="_blank">
							{{ $item->click_url }}
						</a>
					</span>
				...
				<td>
					<span class="no-margin kq_search">{{ $item->click_ip_address }}</span>
				...
				<td>
                    <span class="no-margin kq_search">{{ $item->getSendingServer() ? $item->getSendingServer()->name : '#' }}</span>
                    <span class="text-muted second-line-mobile">{{ trans('messages.sending_server') }}</span>
                </td>
				<td>
					<span class="no-margin kq_search">{{ Auth::user()->customer->formatDateTime($item->click_at, 'datetime_full') }}</span>
					<span class="text-muted second-line-mobile">{{ trans('messages.created_at') }}</span>
				</td>
				@if ($item->campaign->campaignWebhooks()->click()->count())
                    <td class="text-end">
