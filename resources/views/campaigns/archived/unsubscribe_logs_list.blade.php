@if ($items->count() > 0)
    ...
    >
        ...
        @foreach ($items as $key => $item)
            <tr>
                <td>
                    <span class="no-margin kq_search">{{ ($item) ? $item->subscriber->email : '#' }}</span>
                    <span class="text-muted second-line-mobile">{{ trans('messages.recipient') }}</span>
                </td>
                <td>
                    <span class="no-margin kq_search">{{ $item->unsubscribe_ip_address }}</span>
                    <span class="text-muted second-line-mobile">{{ trans('messages.ip_address') }}</span>
                </td>
                <td>
                    <span class="no-margin kq_search">{{ ( is_null($item) || is_null($item->campaign)) ? 'N/A' : $item->campaign->name }}</span>
                    <span class="text-muted second-line-mobile">{{ trans('messages.campaign') }}</span>
                </td>
                <td>
                    <span class="no-margin kq_search">{{ ( $item && $item->getSendingServer()) ? $item->getSendingServer()->name : '#' }}</span>
                    <span class="text-muted second-line-mobile">{{ trans('messages.sending_server') }}</span>
                </td>
                <td>
                    <span class="no-margin kq_search">{{ Auth::user()->customer->formatDateTime($item->unsubscribe_at, 'datetime_full') }}</span>
