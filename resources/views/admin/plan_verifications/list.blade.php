            ...
            <tr class="position-relative">
                    ...
                    <div class="text-nowrap d-flex align-items-canter">
                        @if (!$plan->visible)
                            <a
                                ...
                                link-method="POST"
                                href="{{ action('Admin\PlanVerificationController@visibleOn', $plan->uid) }}"
                        ...
                        @else
                            <a
                                ...
                                link-method="POST"
                                href="{{ action('Admin\PlanVerificationController@visibleOff', $plan->uid) }}"
                ...
                <td>
                    <h5 class="m-0 text-bold">
                        <span class="kq_search d-block" href="{{ action('Admin\PlanVerificationController@edit', $plan->uid) }}">
                            {{ $plan->name }}
                        </span>
                    </h5>
                ...
                <td>
                    <h5 class="no-margin text-bold kq_search">
                        {{ number_with_delimiter($plan->email_verification_credits) }}
                    </h5>
                    <span class="text-muted">{{ trans('messages.plan.email_verification_credits') }}</span>
                </td>
                <td>
                    <h5 class="no-margin text-bold kq_search">
                ...
                <td class="text-end text-nowrap pe-0" width="5%">
                    @can('update', $plan)
                        <a href="{{ action('Admin\PlanVerificationController@edit', $plan->uid) }}" role="button" class="btn btn-secondary btn-icon"> <span class="material-symbols-rounded">edit</span> {{ trans('messages.edit') }}</a>
                    @endcan
                                ...
                                @can('delete', $plan)
                                    <li>
                                        <a class="dropdown-item list-action-single"
                                            link-method="POST"
                                            link-confirm="{{ trans('messages.plan.delete.confirm') }}"
                                            href="{{ action('Admin\PlanVerificationController@delete', $plan->uid) }}"
                                            title="{{ trans('messages.delete') }}" class=""
                                        >
