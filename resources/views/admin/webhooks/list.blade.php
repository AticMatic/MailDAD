        ...
        @foreach ($webhooks as $key => $webhook)
            <tr class="position-relative">
                ...
                @if ($webhook->httpConfig)
                    <td>
                        <a class="d-block fw-semibold" data-control="webhook-job" href="{{ action('Admin\HttpConfigController@httpRequests', $webhook->httpConfig->uid) }}">
                            <span>{{ $webhook->httpConfig->httpRequests()->count() }}</span>
                        </a>
                        <span class="text-muted2">{{ trans('messages.webhook.runs') }}</span>
                    </td>
                    <td>
                        <h5 class="m-0" style="line-height: 22px;">
                            <code class="fw-normal" style="white-space:nowrap;text-overflow:ellipsis;max-width:250px;display:block;overflow:hidden;"><span class="text-uppercase">{{ $webhook->httpConfig->request_method }}</span> {{ $webhook->httpConfig->request_url }}</code>
                        </h5>
                        <span class="text-muted2">{{ trans('messages.webhook.desc') }}</span>
                    </td>
                @else
                    <td colspan="2">
                        <span class="text-muted2">{{ trans('messages.webhook.not_setup_yet') }}</span>
                    </td>
                @endif
                <td>
                ...
                <td class="text-end text-nowrap pe-0">
                    @if (Auth::user()->admin->can('viewLog', $webhook))
                        <a data-control="webhook-job" href="{{ action('Admin\HttpConfigController@httpRequests', $webhook->httpConfig->uid) }}" title="{{ trans('messages.edit') }}" role="button" class="btn btn-secondary btn-icon">
                    ...
                    @if (Auth::user()->admin->can('delete', $webhook) || Auth::user()->admin->can('disable', $webhook) || Auth::user()->admin->can('enable', $webhook))
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end">
                                ...
                                @endif
                                @if (Auth::user()->admin->can('test', $webhook))
                                    <li>
                                        <a class="dropdown-item" data-control="test-webhook" href="{{ action('Admin\HttpConfigController@test', $webhook->httpConfig->uid) }}">
