            ...
            <div class="mc-row d-flex align-items-center">
                ...
                <div class="actions-points">
                    @if (!is_null($trigger))
                        ...
                        @endforeach
                    @elseif ($automation->isDateOfBirthTrigger())

                        @if (is_null($contact->dob))
                            <span title="{{ trans('messages.automation.trigger.date_of_birth.empty_dob') }}">#</span>
                        @else
                            <span title='{{ trans('messages.automation.trigger.date_of_birth.why_not_triggered', [
                                'dob' => $contact->dob,
                                'triggeron' => $contact->dob_to_trigger,
                                'x' => $contact->date_modify,
                                't' => $contact->timediff,
                                'diff' => $contact->datediff,
                            ]) }}'>{{trans('messages.automation.trigger.date_of_birth.wait_until', [ 'w' => $contact->dob_to_trigger, 'diff' => $contact->datediff ])}}</span>
                        @endif
                    @else
