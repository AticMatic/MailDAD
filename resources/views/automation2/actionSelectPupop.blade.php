...
@section('content')
	<div class="row">
        <div class="col-md-12">
            ...
            <div class="line-list">
                ...
                <div action-select="wait_until" data-key="wait" class="d-flex align-items-center line-item action-select-but">
                    <div class="line-icon">
                        <img width="30px" class="icon-img d-inline-block" src="{{ url('images/icons/wait_until.svg') }}" />
                    </div>
                    <div class="line-body">
                        <h5>{{ trans('messages.automation.action.wait_until') }}</h5>
                        <p>{{ trans('messages.automation.action.wait_until.desc') }}</p>
                    </div>
                </div>
    ...
    <script>
        $(() => {
            ...
            $('[action-select="wait_until"]').click(function() {
                // show select trigger confirm box
                openCreateWaitUntilActionPopup('wait_until');
            });
